...
Above code showing a simplest custom middleware it does nothing except execute the next middleware and pass the result into the previous middleware. 
...
`Invocation` is an object represent of the next process will be invoked by the middleware. It has `proceed()` method used to execute the next process and returned the execution result `Promise<ActionResult>`. Invocation has `ctx` property which is the Context useful to get current request information.  
...
9. `ctx.parameters` array of value that will be bound to controller's method. The value arranged in a correct order match with methods parameter.
...
1. Custom global middleware middleware registered using Plumier application during the application bootstrap.
...
## Custom Middleware
There are two kind of custom middleware: Global middleware and Controller/method middleware, both actually the same except in controller/method middleware there are some distinction: 
