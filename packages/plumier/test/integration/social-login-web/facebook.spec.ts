...
import { FacebookOAuthFacility, OAuthFacility, OAuthUser, redirectUri } from "@plumier/social-login"
...
function appStub(opt?: { controller?: Class, csrfEndPoint?: string, loginEndPoint?: string, profileParams?: {} }) {
    ...
    return fixture(opt?.controller ?? AuthController)
        ...
        .set(new FacebookOAuthFacility({ clientId: "123456", clientSecret: "super-secret", loginEndPoint: opt?.loginEndPoint, profileParams: opt?.profileParams }))
        .initialize()
}
...
function mockAxios() {
    ...
    (axios.get as jest.Mock).mockReturnValue(axiosResult(fbProfile));
}

describe("FacebookOAuthFacility", () => {
    it("Should throw when no redirect uri handler provided", async () => {
        class AuthController { }
        expect(appStub({ controller: AuthController }))
            .rejects.toThrowError("No Facebook redirect uri handler found")
    })

    it("Should throw when provided parameterized redirect uri handler", async () => {
        class AuthController {
            @redirectUri("Facebook")
            @route.get("/auth/facebook/callback/:id")
...
describe("Login Endpoint", () => {
    it("Should serve facebook login endpoint", async () => {
        const app = await appStub()
        const request = supertest.agent(app.callback())
        await request.get("/auth/csrf-secret")
        await request.get("/auth/facebook/login")
            .expect(302)
    })

    it("Should redirected to facebook", async () => {
        const app = await appStub()
        const request = supertest.agent(app.callback())
        await request.get("/auth/csrf-secret")
        ...
        expect(url.origin + url.pathname).toBe("https://www.facebook.com/v4.0/dialog/oauth")
    })
    ...
    it("Should detect redirect uri if changed", async () => {
        class AuthController {
            @redirectUri("Facebook")
            @route.get("/auth/facebook/redirect-uri")
            callback() {
    ...
    it("Should add state parameter which valid with csrf secret", async () => {
        ...
        const redirect = await request.get("/auth/facebook/login")
            .expect(302)
    ...
    it("Should able to include other parameters from login endpoint", async () => {
        ...
        const request = supertest.agent(app.callback())
        await request.get("/auth/csrf-secret")
        const resp = await request.get("/auth/facebook/login?foo=bar&lorem=ipsum")
            .expect(302)
        const url = new URL(resp.header["location"])
    ...
    it("Should able to change the login endpoint", async () => {
        const app = await appStub({ loginEndPoint: "/auth/facebook-login" })
        const request = supertest.agent(app.callback())
        await request.get("/auth/csrf-secret")
        const resp = await request.get("/auth/facebook-login")
            .expect(302)
...
describe("Redirect URI Handler", () => {
    ...
    it("Should return 400 when no state parameter provided", async () => {
        ...
        const app = await appStub({ controller: AuthController })
        const request = await supertest.agent(app.callback())
        await request.get(`/auth/facebook/callback?code=lorem`)
    ...
    it("Should return 400 when no authorization code specified", async () => {
        ...
        const app = await appStub({ controller: AuthController })
        const request = await supertest.agent(app.callback())
        await request.get(`/auth/facebook/callback?state=lorem`)
    ...
    it("Should return 400 when no csrf secret on request", async () => {
        ...
        const app = await appStub({ controller: AuthController })
        const request = await supertest.agent(app.callback())
        await request.get(`/auth/facebook/callback?code=lorem&state=Facebook.ipsum`)
            .expect(400, { status: 400, message: "Request doesn't contains csrf secret" })
    })
    ...
    it("Should return 400 when provided invalid csrf token", async () => {
        ...
        await request.get(`/auth/facebook/callback?code=lorem&state=Facebook.lorem`)
